...
### 🧠 長期記憶活用 (OpenClaw LTM、プロジェクトに長期記憶スキルがあれば任意)
Use OpenClaw LTM (or the project's equivalent long-term memory skill, if any) for persistent knowledge management via Bash. プロジェクトに該当スキルが無ければこの節はスキップしローカル調査のみで代替する:
- **Knowledge Search**: Use `ltm_search.py` for past investigation results
- **Latest Entries**: Use `ltm_latest.py` for recent knowledge
- **Knowledge Storage**: Use `POST /hooks/<project>-memory` to save investigation insights
...
2. **   curl -sS -X POST http://host.docker.internal:18789/hooks/<project>-memory \
     -H "Authorization: Bearer $HOOK_TOKEN" \
     -H "Content-Type: application/json" \
     -d '{
       "title": "[Project] Investigation: [Topic]",
