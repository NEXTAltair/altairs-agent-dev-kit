...
7. **モデル既定挙動・lint との重複**: rules / CLAUDE.md の各節が、lint (Ruff / mypy) が検出する
   事項 (型ヒント構文、import 順序、行長、危険関数) や、現行モデルが指示なしで既に行う挙動
   (テスト実行、根本原因修正、スコープ厳守) を再掲していないかを見る。モデル更新のたびに
   陳腐化する足場なので、3~6 か月ごとに容疑化して Phase 2 送り
...
| 完了済み計画ファイル | 削除 |
| lint / モデル既定挙動を再掲した rules・CLAUDE.md 節 | 削除、または毎回強制したいものは hook へ変換 |
