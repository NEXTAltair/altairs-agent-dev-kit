...
別途 clone した kit で `git checkout v0.5.0` を実行し、次を実行する (Linux は `python3`):
...
```bash
npx skills add "github:NEXTAltair/altairs-agent-dev-kit#v0.5.0" --skill check-existing
npx skills add "github:NEXTAltair/altairs-agent-dev-kit#v0.5.0" --skill pr-maintainer
npx skills add "github:NEXTAltair/altairs-agent-dev-kit#v0.5.0" --skill okf-bundle
```
